...
/**
 * JsonThings Fabric 主入口（common 部分），对应上游官方 gigaherz JsonThings 1.21.1 {@code JsonThings}。
 *
 ...
 *   <li>{@code ClientHandlers}（颜色 init / 渲染层 / 颜色注册）→ {@link JsonThingsClient} +
 *       {@code CLIENT_STARTED}（等价 enqueueWork 到主线程）；客户端入口必须与 main 入口分开，
 *       否则专用服务端加载 main 时会因引用 {@code net.minecraft.client.*} 而失败。</li>
 * </ul>
 *
 * <p>与 NeoForge 版差异说明：
 * <ul>
 *   <li>Neo {@code NamedRenderTypeManager.get(layer).block()}（数据包自定义命名渲染层）→ 仅支持 vanilla 渲染层名
 *       （solid/cutout_mipped/cutout/translucent/tripwire），见 {@link JsonThingsClient}；</li>
